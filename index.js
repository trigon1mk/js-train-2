...
function getIntegerFromString(s) {
  let numParsentInt = parseInt(s);

  if (isNaN(numParsentInt)) {
    return "Couldn't extract an integer from the provided string.";
  } else {
    return numParsentInt;
  }
...
function encodeURLComponent(urlComponent) {
  let encode = encodeURIComponent(urlComponent);
  return encode;
...
function decodeURLComponent(urlComponent) {
  let decode = decodeURIComponent(urlComponent);
  return decode;
